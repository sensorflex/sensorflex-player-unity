// FrameLoader.cs — ring-buffer state owned by each backend.
//
// IFrameLoaderState + FrameLoaderState are the only types here.
// Backends create a FrameLoaderState in their StartLoading() call and
// expose it via ISessionBackend.State; SessionLoader delegates all
// runtime frame queries through that interface.
...
namespace SensorFlex.Player.Library
{
    ...
    internal sealed class FrameLoaderState : IFrameLoaderState
    {
        ...
        public FrameLoaderState(int bufSize) { BufSize = bufSize; }
